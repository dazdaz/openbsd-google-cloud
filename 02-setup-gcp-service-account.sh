...
setup() {
    ...
    echo ""
    echo "After completing the above steps, deploy OpenBSD to GCP:"
    echo -e "   ${YELLOW}./03-deploy-gcp.sh --image-file build/artifacts/openbsd-7.8.raw.gz --project-id ${PROJECT_ID}${NC}"
    echo ""
...
check() {
    ...
    if [ "$all_good" = true ]; then
        ...
        echo ""
        echo "Your GCP setup is complete. You can now deploy:"
        echo -e "   ${YELLOW}./03-deploy-gcp.sh --image-file build/artifacts/openbsd-7.8.raw.gz --project-id ${PROJECT_ID}${NC}"
        echo ""
        return 0
    else
